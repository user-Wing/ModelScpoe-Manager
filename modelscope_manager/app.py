...
from PySide6.QtCore import (
    QEasingCurve, QEvent, QProcess, QPropertyAnimation,
...
from .database import AccountRecord, AccountStore, IndexedEntry, classify_file, initialize_database
...
def thumbnail_batch_policy(entry_count: int) -> tuple[int, int, int]:
    ...
    return (4, 2, 900) if entry_count > 100 else (16, 4, 250)
...
class NavButton(QPushButton):
    def __init__(self, icon: str, text: str, icon_size: int = 13, icon_x: int = 13, parent: QWidget | None = None):
        super().__init__(f"    {text}", parent, objectName="navButton")
    ...
    def resizeEvent(self, event) -> None:
        self.icon_label.move(self.icon_x, max(0, (self.height() - self.icon_label.sizeHint().height()) // 2))
...
class ThumbnailThread(QThread):
    ...
    def _create_thumbnail(self, entry: RemoteEntry, ffmpeg: str | None) -> tuple[str, str] | None:
        if self.isInterruptionRequested() or not ffmpeg:
            return None
        key = hashlib.sha256(f"16x9-v2/{self.repo.repo_type}/{self.repo.repo_id}/{entry.path}".encode()).hexdigest()
        ...
        try:
            if not target.exists():
                ...
                if Path(entry.path).suffix.lower() in IMAGE_EXTENSIONS:
                    ...
                    subprocess.run(command + ["-f", "image2pipe", "-i", "pipe:0", "-frames:v", "1", "-vf", "scale=208:117:force_original_aspect_ratio=decrease,pad=208:117:(ow-iw)/2:(oh-ih)/2:color=black", str(target)], input=image_bytes, capture_output=True, timeout=45, check=True, creationflags=creationflags)
                else:
                    subprocess.run(command + ["-ss", "0", "-probesize", "32k", "-analyzeduration", "0", "-i", url, "-frames:v", "1", "-an", "-vf", "scale=208:117:force_original_aspect_ratio=decrease,pad=208:117:(ow-iw)/2:(oh-ih)/2:color=black", str(target)], capture_output=True, timeout=45, check=True, creationflags=creationflags)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.thumbnail_timer.setInterval(900)
    ...
    def _build_ui(self) -> None:
        ...
        self.nav_sidebar = nav
        nav.setMinimumWidth(58)
        nav.setMaximumWidth(190)
        nav_layout = QVBoxLayout(nav)
        nav_layout.setContentsMargins(14, 20, 14, 18)
        nav_layout.setSpacing(6)
        self.brand = QLabel("ModelScope\nManager", objectName="brand")
        self.brand.setFixedHeight(self.brand.sizeHint().height())
        nav_layout.addWidget(self.brand)
        self.nav_toggle_button = QPushButton("☰", objectName="navToggle")
        self.nav_toggle_button.setToolTip("收起侧栏")
        self.nav_toggle_button.clicked.connect(self._toggle_sidebar)
        nav_layout.addWidget(self.nav_toggle_button, 0, Qt.AlignmentFlag.AlignHCenter)
        nav_layout.addSpacing(22)
        self.nav_buttons: list[QPushButton] = []
        for page, icon, label in ((0, "▣", "资源管理"), (3, "⌕", "资源搜索"), (1, "⇅", "传输列表"), (4, "▤", "备份文件夹"), (5, "▧", "图床")):
            button = NavButton(icon, label, 20)
            button.setCheckable(True)
            button.setProperty("pageIndex", page)
            button.setProperty("navText", label)
            button.clicked.connect(lambda checked=False, target=page: self._navigate(target))
            nav_layout.addWidget(button)
            self.nav_buttons.append(button)
        nav_layout.addStretch()
        settings_nav = NavButton("⚙", "设置", 13, 13)
        settings_nav.setCheckable(True)
        settings_nav.setProperty("pageIndex", 2)
        settings_nav.setProperty("navText", "设置")
        ...
        self.nav_footer = QLabel("1.0.1", objectName="navFooter")
        self.nav_footer.setFixedHeight(self.nav_footer.sizeHint().height())
        self.nav_footer.setAlignment(Qt.AlignmentFlag.AlignCenter)
        nav_layout.addWidget(self.nav_footer)
        shell.addWidget(nav)
        ...
        self.remote_thumbnail_list = QListWidget()
        ...
        settings_layout.addLayout(settings_notice_row)
        ...
        self.gpu_acceleration_checkbox = QCheckBox("启用 GPU 加速（重启后生效）")
        ...
        self.acrylic_checkbox = QCheckBox("启用 Blur 亚克力半透明效果")
        ...
        self.thumbnail_workers.setValue(4)
        ...
        self.copy_threshold_unit = QComboBox()
        self.copy_threshold_unit.addItem("MB", 1024 ** 2)
        self.copy_threshold_unit.addItem("GB", 1024 ** 3)
        ...
        self.alist_auto_start = QCheckBox("启动程序后自动启动监听")
        ...
        for card in (general_card, theme_card, token_card, download_card, player_card, alist_card, index_card):
    ...
    def _restore_settings(self) -> None:
        ...
        self.thumbnail_workers.setValue(int(self.settings.value("preview/thumbnail_workers", 4)))
        self.copy_threshold_value.setValue(float(self.settings.value("copy/threshold_value", 100.0)))
        unit_index = self.copy_threshold_unit.findData(int(self.settings.value("copy/threshold_unit", 1024 ** 2)))
        self.copy_threshold_unit.setCurrentIndex(max(0, unit_index))
    ...
    def _toggle_sidebar(self) -> None:
        ...
        start_width = self.nav_sidebar.width()
        target_width = 58 if collapsed else 190
        self.brand.setText("" if collapsed else "ModelScope\nManager")
        self.nav_footer.setText("" if collapsed else "1.0.1")
        self.nav_toggle_button.setText("☰")
        self.nav_toggle_button.setToolTip("展开侧栏" if collapsed else "收起侧栏")
        for button in self.nav_buttons:
            button.setText("" if collapsed else f"    {button.property('navText') or button.text().strip()}")
            button.icon_label.setVisible(True)
            if collapsed:
                button.setToolTip(button.property("navText") or button.toolTip())
        ...
        animation = QPropertyAnimation(self.nav_sidebar, b"maximumWidth", self)
        ...
        def finished() -> None:
            if self.sidebar_animation is animation:
    ...
    def _reset_thumbnail_queue(self, entries: list[RemoteEntry]) -> None:
        ...
        if self.thumbnail_queue:
            self.thumbnail_timer.start(1200)
    ...
    def _load_visible_thumbnails(self) -> None:
        ...
        if idle_for < 0.75:
            self.thumbnail_timer.start(max(100, int((0.75 - idle_for) * 1000)))
            return
    ...
    def show_resource_search(self) -> None:
        ...
        query = QLineEdit()
        query.setPlaceholderText("搜索所有已索引文件")
    ...
    def _apply_window_effects(self, dark: bool | None = None, acrylic: bool | None = None) -> None:
        ...
        self.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground, acrylic)
        if not self.isVisible():
            return
        applied = apply_windows_backdrop(int(self.winId()), acrylic, dark)
        if hasattr(self, "graphics_status"):
            if acrylic and applied:
                self.graphics_status.setText("亚克力半透明由 Windows GPU 合成器处理；GPU 渲染切换重启后生效。")
            elif acrylic:
                self.graphics_status.setText("当前系统不支持原生亚克力，已回退为普通半透明；GPU 渲染切换重启后生效。")
            else:
                self.graphics_status.setText("亚克力效果已关闭；GPU 渲染切换重启后生效。")
    ...
    def _set_current_directory(self, path: str) -> None:
        self.current_directory_path = path.strip("/")
        self.target_edit.setText(self.current_directory_path)
        self.resource_path_label.setText(f"/ {self.current_directory_path}" if self.current_directory_path else self._t("/ 根目录"))
    ...
    def _copy_remote_link(
        ...
        try:
            if entry.is_dir:
                ...
                public = str(repo.visibility).casefold() == "public" or not service.token
    ...
    def _paste_remote_copy(self, destination_service: ModelScopeService, destination_repo: Repository, destination_folder: str) -> None:
        ...
        if selected.is_dir:
            total_size = self.folder_index.update_folder(
                source_repo, selected.path, source_entries, source_repo.visibility == "public" or source_service.token == "",
            )
        else:
    ...
    def _public_search_sort_combo_changed(self) -> None:
    ...
    def _repository_paths_dropped(self, raw_paths: list[str], directory: RemoteEntry) -> None:
        ...
        self.add_paths(raw_paths, directory.path)
        if not any(item.status == "waiting" for item in self.upload_items):
            return
        self._navigate(1)
    ...
    def add_paths(self, raw_paths: list[str], target: str | None = None) -> None:
        try:
